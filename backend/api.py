...
def get_seasons_episodes(season_num):
    if len(episodes) < season_num:
        abort(404)
    response = jsonify(episodes[season_num - 1])
    response.headers['Access-Control-Allow-Origin'] = '*'
    return response
...
def get_episode(season_num, epsiode_num):
    ...
    if len(detailed_episodes[season_num - 1]) < epsiode_num:
        abort(404)
    response = jsonify(detailed_episodes[season_num - 1][epsiode_num - 1])
    response.headers['Access-Control-Allow-Origin'] = '*'
    return response
...
def get_random_episode():
    ...
    response = jsonify(random.choice(random.choice(episodes)))
    response.headers['Access-Control-Allow-Origin'] = '*'
    return response
